...
if ( ! class_exists( 'BuilderImportExport' ) ) {
	class BuilderImportExport {
		...
		function show_data_source_import_methods( $form ) {
			...
			foreach ( (array) $info['data_sources'] as $var => $data_source ) {
				...
				if ( isset( $this->_data_sources[$var] ) && is_callable( array( $this->_data_sources[$var], 'show_import_methods_form' ) ) )
					...
					echo "<p>" . __( 'The plugin or component that this data is for is not present on this site. This data will not be imported.', 'it-l10n-Builder-Everett' ) . "</p>\n";
			...
			foreach ( (array) $info['data_sources'] as $var => $data_source ) {
				...
				if ( isset( $this->_data_sources[$var] ) ) {
					if ( 'skip' === $submission_data['data_sources'][$var]['method'] )
						echo '<p>' . sprintf( __( 'The %1$s data from the export file will not be imported to the site.', 'it-l10n-Builder-Everett' ), $this->_data_sources[$var]->get_name() ) . "</p>\n";
					else {
						...
						if ( false === $has_custom_process ) {
							if ( 'replace' === $submission_data['data_sources'][$var]['method'] )
								printf( __( 'The %1$s data on the site will be removed and replaced with the %1$s data from the export file.', 'it-l10n-Builder-Everett' ), $this->_data_sources[$var]->get_name() );
							else
								_e( 'The data will be imported as selected.', 'it-l10n-Builder-Everett' );
						}
					}
		...
		function _import_file( $file ) {
			...
			if ( ! is_file( $file ) ) {
				$this->add_error( new WP_Error( 'builder-import-export-no-file', __( 'Unable to find requested export file', 'it-l10n-Builder-Everett' ) ) );
				return false;
			}
			...
			if ( is_wp_error( $result ) || ! is_file( "$path/info.txt" ) || ! is_dir( "$path/data-sources" ) ) {
				...
				$this->add_error( new WP_Error( 'builder-import-export-invalid-export-format', __( 'The uploaded file is not a valid Builder export file. It cannot be imported.', 'it-l10n-Builder-Everett' ) ) );
				return false;
			}
