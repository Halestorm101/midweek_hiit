...
final class ITSEC_Strong_Passwords {
	...
	public function filter_user_profile_update_errors( $errors, $update, $user ) {
		...
		if ( $this->fails_enforcement( $user, $caps, $password_strength ) ) {
			if ( $update ) {
				$errors->add( 'pass', wp_kses( __( '<strong>Error</strong>: Due to site rules, a strong password is required. Please choose a new password that rates as <strong>Strong</strong> on the meter. The user changes have not been saved.', 'it-l10n-ithemes-security-pro' ), array( 'strong' => array() ) ) );
			} else {
				$errors->add( 'pass', wp_kses( __( '<strong>Error</strong>: Due to site rules, a strong password is required. Please choose a new password that rates as <strong>Strong</strong> on the meter. The user has not been created.', 'it-l10n-ithemes-security-pro' ), array( 'strong' => array() ) ) );
			}
		}
	...
	public function validate_password_reset( $errors, $user ) {
		...
		if ( $this->fails_enforcement( $user, $caps ) ) {
			$errors->add( 'pass', wp_kses( __( '<strong>Error</strong>: Due to site rules, a strong password is required. Please choose a new password that rates as <strong>Strong</strong> on the meter. The password has not been updated.', 'it-l10n-ithemes-security-pro' ), array( 'strong' => array() ) ) );
		}
	}
